...
def ensure_base_state(
    ...
    if (
        "active_session_id" not in state
        or state["active_session_id"] not in sessions_by_id
    ):
...
def sync_prompt_selection_state(
    ...
    if (
        ...
        and state.get("selected_prompt_template_id") not in prompt_templates_by_id
    ):
        state["selected_prompt_template_id"] = next(iter(prompt_templates_by_id))
    ...
    if state["system_prompt_session_id"] != current_session.id:
        ...
        if "system_prompt_draft" not in state:
            state["system_prompt_draft"] = (
                legacy_system_prompt_input or current_session.system_prompt
            )
        if not current_preview_prompt and previous_preview_prompt:
            # 从预览切回编辑时刷新编辑器 key，强制文本框以当前草稿重新初始化。
            state["system_prompt_editor_nonce"] = (
                state.get("system_prompt_editor_nonce", 0) + 1
            )
        ...
        if (
            ...
            and not state["system_prompt_draft"]
            and current_session.system_prompt
        ):
            state["system_prompt_draft"] = current_session.system_prompt
